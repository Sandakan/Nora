...
export function useMediaSession(player: HTMLAudioElement, dependencies: MediaSessionDependencies) {
	...
	const { toggleSongPlayback, handleSkipBackwardClick, handleSkipForwardClick, updateSongPosition } = dependencies;
	...
	useEffect(() => {
		...
		const safeSetPositionState = () => {
			if (Number.isFinite(player.duration) && player.duration > 0 && Number.isFinite(player.currentTime)) {
		...
		const updateMediaSessionMetaData = () => {
			...
			if (typeof MediaMetadata !== 'undefined') {
				mediaSession.metadata = new MediaMetadata({
					...
					album: currentSong.album ? currentSong.album.name || t('common.unknownAlbum') : t('common.unknownAlbum'),
					artwork,
				});
			}
	...
	}, [handleSkipBackwardClick, handleSkipForwardClick, t, toggleSongPlayback, updateSongPosition, player]);
}
