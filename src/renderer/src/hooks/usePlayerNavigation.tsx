...
export function usePlayerNavigation(
	...
	recordListeningData: (songId: number, songDuration: number, repetition?: boolean, isKnownSource?: boolean) => void,
) {
	// Support both AudioPlayer instance and HTMLAudioElement for backward compatibility
	const player = playerInstance instanceof HTMLAudioElement ? playerInstance : (playerInstance as AudioPlayer).audio;
	const audioPlayer = playerInstance instanceof HTMLAudioElement ? null : (playerInstance as AudioPlayer);
	...
	const handleSkipForwardClick = useCallback(
		(reason: SongSkipReason = 'USER_SKIP') => {
			...
			if (store.state.player.isRepeating === 'repeat-1' && reason !== 'USER_SKIP') {
				...
				recordListeningData(store.state.currentSongData.songId, store.state.currentSongData.duration, true);
