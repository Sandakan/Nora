...
import { queryClient } from 'src/renderer/src';
...
function FavoritesPlaylistInfoPage() {
	...
	const playlistSortingState = useStore(store, (state) => state.localStorage.sortingStates?.songsPage || 'addedOrder');
	const preferences = useStore(store, (state) => state.localStorage.preferences);
	const { updateQueueData, addNewNotifications, createQueue, playSong } = useContext(AppUpdateContext);
	...
	const handleSongPlayBtnClick = useCallback(
		(currSongId: number) => {
			const queueSongIds = favoriteSongs.filter((song) => !song.isBlacklisted).map((song) => song.songId);
	...
	const addSongsToQueue = useCallback(() => {
		const validSongIds = favoriteSongs.filter((song) => !song.isBlacklisted).map((song) => song.songId);
	...
	const importSongsToFavorites = useCallback(() => {
		window.api.playlistsData
			...
			.then(() => {
				queryClient.invalidateQueries({ queryKey: songQuery.favorites({ sortType: sortingOrder }).queryKey });
			})
			.catch((err) => console.error(err));
	}, [sortingOrder]);
	...
	return (
		<MainContainer
			...
			<VirtualizedList
				...
				components={{
					Header: () => <PlaylistInfoAndImgContainer playlist={playlistData} songs={favoriteSongs} />,
				}}
