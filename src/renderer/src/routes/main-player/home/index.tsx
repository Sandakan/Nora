...
const AddMusicFoldersPrompt = lazy(() => import('@renderer/components/MusicFoldersPage/AddMusicFoldersPrompt'));

function HomePage() {
	const { updateContextMenuData, changePromptMenuData, addNewNotifications } = useContext(AppUpdateContext);
	...
	const { noOfRecentlyAddedSongCards, noOfRecentandLovedArtists, noOfRecentandLovedSongCards } = useMemo(() => {
	...
	const importAppData = useCallback(
		(_: unknown, setIsDisabled: (state: boolean) => void, setIsPending: (state: boolean) => void) => {
			setIsDisabled(true);
			setIsPending(true);
							...
							handlerFunction: () =>
								changePromptMenuData(
									...
									<ErrorPrompt
										...
										message={
											<>
												This prompt is used to develop the prompt menu. Instead of clicking doing something that will
												open a prompt and also result in consequences, this approach is much better because nothing will
												change when opening this prompt.
											</>
										}
										showSendFeedbackBtn
									/>,
	...
	return (
		<MainContainer
			...
			onContextMenu={(e) => {
				e.preventDefault();
				e.stopPropagation();
				if (homePageContextMenus.length > 0) updateContextMenuData(true, homePageContextMenus, e.pageX, e.pageY);
			}}
			...
			<>
				...
				{recentlyAddedSongsContainerRef.current && (
					<>
						{latestSongs[0] !== null && (
							<RecentlyAddedSongs latestSongs={latestSongs} noOfVisibleSongs={noOfRecentlyAddedSongCards} />
						)}
						<RecentlyPlayedSongs
						...
						<MostLovedSongs
							...
							noOfVisibleSongs={noOfRecentandLovedSongCards}
						/>
						<MostLovedArtists mostLovedArtists={mostLovedArtists} noOfVisibleArtists={noOfRecentandLovedArtists} />
					</>
				)}
