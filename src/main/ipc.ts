...
import {} from './db/queries/userPreferences';
...
export function initializeIPC(mainWindow: BrowserWindow, abortSignal: AbortSignal) {
	if (mainWindow) {
		...
		ipcMain.on('app/setDiscordRpcActivity', (_: unknown, options: unknown) => setDiscordRpcActivity(options));
		...
		ipcMain.handle('app/getSongFromUnknownSource', (_, songPath: string) => sendAudioDataFromPath(songPath));
		...
		ipcMain.handle('app/getAllHistorySongs', (_, sortType?: SongSortTypes, paginatingData?: PaginatingData) =>
			getAllHistorySongs(sortType, paginatingData),
		);

		ipcMain.handle('app/getAllFavoriteSongs', (_, sortType?: SongSortTypes, paginatingData?: PaginatingData) =>
		...
		// );
		ipcMain.handle('app/saveUserSettings', (_, settings: Partial<UserSettings>) => saveUserSettings(settings));
		...
		ipcMain.handle('app/removeIgnoredArtist', (_, artistId: number) => removeIgnoredArtist(artistId));
		...
		ipcMain.handle('app/addIgnoredFeaturingArtist', (_, artistId: number) => addIgnoredFeaturingArtist(artistId));

		ipcMain.handle('app/removeIgnoredFeaturingArtist', (_, artistId: number) => removeIgnoredFeaturingArtist(artistId));
		...
		ipcMain.handle('app/getAlbumInfoFromLastFM', (_, albumId: number) => getAlbumInfoFromLastFM(albumId));
		...
		ipcMain.handle('app/sendNowPlayingSongDataToLastFM', (_, songId: number) => sendNowPlayingSongDataToLastFM(songId));

		ipcMain.handle('app/getArtistArtworks', (_, artistId: number) => getArtistInfoFromNet(artistId));
		...
		ipcMain.handle('app/searchSongMetadataResultsInInternet', (_, songTitle: string, songArtists: string[]) =>
			searchSongMetadataResultsInInternet(songTitle, songArtists),
		);

		ipcMain.handle('app/fetchSongMetadataFromInternet', (_, source: SongMetadataSource, sourceId: string) =>
		...
		ipcMain.handle('app/getArtistDuplicates', (_, artistName: string) => getArtistDuplicates(artistName));

		ipcMain.handle('app/resolveArtistDuplicates', (_, selectedArtistId: number, duplicateIds: number[]) =>
			resolveArtistDuplicates(selectedArtistId, duplicateIds),
		);

		ipcMain.handle('app/resolveSeparateArtists', (_, separateArtistId: number, separateArtistNames: string[]) =>
		...
		ipcMain.handle('app/getQueueInfo', (_, queueType: QueueTypes, id: string) => getQueueInfo(queueType, id));

		ipcMain.handle('app/addNewPlaylist', (_, playlistName: string, songIds?: string[], artworkPath?: string) =>
			addNewPlaylist(playlistName, songIds, artworkPath),
		);

		ipcMain.handle('app/removePlaylists', (_, playlistIds: number[]) => removePlaylists(playlistIds));
		...
		ipcMain.handle('app/deleteSongsFromSystem', (_, absoluteFilePaths: string[], isPermanentDelete: boolean) =>
		...
		ipcMain.handle('app/restoreBlacklistedSongs', (_, songIds: number[]) => restoreBlacklistedSongs(songIds));
		...
		ipcMain.handle('app/clearSearchHistory', (_, searchText?: string[]) => clearSearchHistoryResults(searchText));
		...
		ipcMain.on('app/revealSongInFileExplorer', (_, songId: number) => revealSongInFileExplorer(songId));

		ipcMain.on('app/revealFolderInFileExplorer', (_, folderPath: string) => shell.showItemInFolder(folderPath));
		...
		ipcMain.handle('app/exportAppData', (_, localStorageData: string) => exportAppData(localStorageData));
		...
		ipcMain.handle('app/removeAMusicFolder', (_, absolutePath: string) => removeMusicFolder(absolutePath));
		...
		ipcMain.handle('app/toggleAutoLaunch', (_, autoLaunchState: boolean) => toggleAutoLaunch(autoLaunchState));
		...
		ipcMain.handle('app/blacklistFolders', (_, folderPaths: string[]) => blacklistFolders(folderPaths));

		ipcMain.handle('app/restoreBlacklistedFolders', (_, folderPaths: string[]) =>
			restoreBlacklistedFolders(folderPaths),
		);

		ipcMain.handle('app/toggleBlacklistedFolders', (_, folderPaths: string[], isBlacklistFolder?: boolean) =>
			toggleBlacklistFolders(folderPaths, isBlacklistFolder),
		);

		ipcMain.on('app/networkStatusChange', (_: unknown, isConnected: boolean) => {
			logger.info(isConnected ? `App connected to the internet successfully` : `App disconnected from the internet`);
