...
const createM3u8FileForPlaylist = async (playlistId: number, playlistName: string, songPaths: string[]) => {
...
const exportPlaylist = async (playlistId: number) => {
	...
	if (playlist.songs.length === 0)
		return logger.warn("Failed to export playlist because requested playlist didn't have any songs.", {
			playlistId,
		});
